...
fn goto_never_truncates_existing_file() {
    ...
    assert!(
        app.panes[0].pending_select.is_some(),
        "the file must be pending selection"
    );
...
#[test]
fn terminal_candidates_includes_known_emulators() {
    let candidates = ira::app::terminal_candidates("/tmp");
    assert!(!candidates.is_empty(), "candidate table must not be empty");
    assert_eq!(candidates[0].0, "foot");
    assert_eq!(
        candidates[0].1,
        vec!["--working-directory".to_string(), "/tmp".to_string()]
    );
    for (program, args) in &candidates {
        if program == "xterm" {
            assert!(args.is_empty(), "xterm inherits the current directory");
        } else {
            assert!(
                args.iter().any(|a| a.contains("/tmp")),
                "{program} must be pointed at the directory: {args:?}"
            );
        }
    }
}
...
#[test]
fn spawn_native_terminal_sets_error_without_folder() {
    let mut app = App::default();
    app.panes[0].folder = None;
    app.spawn_native_terminal();
    let status = app.status.as_ref().expect("an error status must be set");
    assert!(status.is_error);
    assert!(
        status.text.contains("No folder"),
