...
protocols (Sixel is skipped on native Windows — it leaves the preview blank;
`IRA_IMAGES` overrides). Protocol-less hosts such as Terminal.app get 2×4 Unicode
braille blocks instead of half-blocks. Decoding happens on a bounded background worker
