...
**Selection & visibility:** `Space` multi-selects (rows show `[*]`/`[ ]`); `Ctrl+A` selects or clears everything, `Alt+I` inverts the selection. (Super-based binds are not reliable in terminals, so `Ctrl`/`Alt` are used instead.) `.` toggles whether hidden (dot) files are listed. Rows carry a per-type icon: Nerd Font glyphs when a font that has them is reachable by the terminal (bundled by kitty/WezTerm/Ghostty/Warp, set as the Windows Terminal or VS Code profile font, or installed on Linux/macOS where the OS falls back per glyph; re-checked every launch, `IRA_ICONS=nerd` forces it); otherwise two-cell color emoji (📁 🦀 🐍 🎬 📦 …, only codepoints Unicode defines as wide, so columns stay put) on terminals known to render them (Windows Terminal, macOS terminals, VS Code, VTE, Konsole, kitty, Alacritty, foot); otherwise a single-width Unicode set (`□` folder, `≡` text, `▶` video, `♪` audio, `▣` archive, `·` other) that renders identically on Linux/macOS/Windows stock fonts. Colors come from the semantic theme: six built-in presets (Catppuccin Mocha, Cyberpunk 2077, Gruvbox Dark, Nord, Dracula, Tokyo Night), truecolor with automatic 256-color quantization on Terminal.app.

**Themes:** `\` switches to the next preset in that order and wraps around; the bottom banner shows `Switched to <name>`, the `Actions` box title shows the active preset, and the choice is saved to `~/.config/ira/state` right away so it survives restarts. A fresh profile starts from `preset = "<id>"` in `~/.config/ira/theme.toml` (default `mocha`); per-key color overrides in that file are re-applied on top of every preset you cycle to. `ira --check-terminal` prints what was detected and which theme is active. See [themes.md](themes.md) for the full list with swatches, font recommendations, and icon sets.

**Edit text in the preview:** with a text file previewed in column mode, `Tab` focuses the editor — type to edit, `Ctrl+S` saves atomically, `Esc` exits — plain letters type. `q` types a `q`; nothing quits while editing. Read-only, binary, non-UTF-8, and >5 MB files open read-only instead. Unsaved changes are discarded on `Esc`/`Tab` (shown by a `*` in the border). **Image preview:** `v` cycles the ACTIVE pane's preview mode — modes are per pane, survive restarts, and switching panes with Tab never changes either pane's mode. **Column** renders the selected file as an image in a side column. **Grid** replaces the file list with a thumbnail grid (image thumbnails, glyphs for folders and unsupported files, cursor highlighted; all formats the list shows are shown, including filtered views). A **text** preview takes two thirds of the pane (two thirds of the app width when both panes are open) so code and notes stay readable. **Details** replaces the file list with name / size / relative-modified columns (`3 days ago`); folders carry no size — recursive measurement is not started in this mode. Rendering picks the best protocol the terminal supports at startup — kitty, iTerm2, or Sixel for true graphics. Native Windows and Terminal.app use a transparent overlay over the preview/thumbnail cells (names and folders stay visible); `IRA_IMAGES=sixel` still forces Sixel. Everywhere else without a protocol uses 2×4 Unicode braille blocks (U+2800–28FF). Override with `IRA_IMAGES=auto|sixel|kitty|iterm2|blocks`. Supported formats: PNG, JPEG, GIF, BMP, WebP — plus MP4/MOV videos and HEIC photos when `ffmpeg` is on `PATH`, and PDF first pages when `pdftoppm` (poppler) is on `PATH` (both optional runtime dependencies; without them those files show glyphs). **Text and code files** (`txt md json toml yaml csv rs py sh c html`…), extensionless files (`Makefile`, `Dockerfile`, `LICENSE`, `configure`, scripts) and dotfiles (`.env`, `.env.local`, `.gitignore`, `.bashrc`) render natively in the column — head of the file, NUL-sniffed so binaries never render as garbage, capped at 256 KB with a truncation notice. Thumbnails for the screens adjacent to the viewport are prefetched in the background, so scrolling large folders feels instant. Decoding runs on a bounded background worker pool and never blocks the UI; thumbnails are cached in memory and on disk (`~/.cache/ira/thumbnails`, keyed by path + mtime + size, so edited images refresh automatically).
  ...
  Previews fall back to the braille underlay when the window cannot be located, the geometry
  sample goes stale (>2 s), or the pane is too wide/tall for one overlay bitmap. On hosts
  whose console cannot be tied to a window — VS Code's integrated terminal on Windows is not
  a console HWND — the frontmost terminal window stands in, so a preview can briefly appear
  over another terminal; with image support enabled VS Code answers the probe with Sixel and
  uses its own protocol instead.
