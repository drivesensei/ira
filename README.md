...
- **Image previews.** Kitty, iTerm2 or Sixel when the terminal answers the startup probe. Native Windows and Terminal.app use a transparent native overlay over the preview cells (Win32 layered window / AppKit panel); names and folders stay visible in grid mode. `IRA_IMAGES=sixel` still forces the protocol. iTerm2, Ghostty, kitty and WezTerm keep their in-terminal protocols. Braille is the last resort if the window cannot be located.
