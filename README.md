...
- **Image previews.** Kitty, iTerm2 or Sixel when the terminal answers the startup probe. Graphics protocols paint on **stdout** (Windows ConPTY only parses Sixel DCS there). Silent probe + `WT_SESSION` on native Windows assumes Sixel at 10×20. Terminal.app has no protocol: a native AppKit overlay sits over the preview cells (2×4 braille underlay if the window cannot be located). iTerm2, Ghostty, kitty and WezTerm keep their in-terminal protocols. `IRA_IMAGES=auto|sixel|kitty|iterm2|blocks` overrides; braille is the last resort.
