...
- **Image previews.** Kitty, iTerm2 or Sixel when the terminal answers the startup probe. Native Windows and Terminal.app use a transparent native overlay over the preview cells (Win32 layered window / AppKit panel); names and folders stay visible in grid mode. The overlay is only shown while ira's terminal window is in front, and it is identified positively on Windows (the console window, its owner, or Windows Terminal's root window) and by matching our tty against the front window's tab on macOS Terminal.app. Where no terminal window can be correlated with our console — VS Code's integrated terminal on Windows is not a console HWND — the frontmost terminal window is used instead, so a preview can briefly appear over another terminal. `IRA_IMAGES=sixel` still forces the protocol. iTerm2, Ghostty, kitty and WezTerm keep their in-terminal protocols (Terminal.app is the macOS overlay host; iTerm2 falls back to the overlay only when it did not answer the probe, e.g. `IRA_IMAGES=blocks`). Braille is the last resort: no terminal window can be located, the geometry sample is stale (>2 s), or the pane is too wide/tall for one overlay bitmap.
